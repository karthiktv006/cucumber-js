function ArgumentParser(argv) {
  ...
  var self = {
    parse: function parse() {
      ...
      self.processRemaining(options.argv.remain);
    },

    processRemaining: function processRemaining(remaining) {
      var files = _.filter(remaining, function (remain) {
        return remain.indexOf('@') > -1;
      });

      files.forEach(function (file) {
        var features = self.getFeaturesFromFilePath(file.split('@')[1]);
        self.getOptions().argv.remain.push(features);
      });

      remaining = _.filter(remaining, function (remain) {
        return remain.indexOf('@') === -1;
      });

      options.argv.remain = _.flatten(remaining);
    },
    ...
    getOutputFile: function getOutputFile() {
      var outputFile = self.getOptionOrDefault(ArgumentParser.OUTPUT_FILE_NAME, ArgumentParser.DEFAULT_OUTPUT_FILE_VALUE);
      return outputFile;
    },
    ...
    getKnownOptionDefinitions: function getKnownOptionDefinitions() {
      ...
      definitions[ArgumentParser.OUTPUT_FILE_NAME]                 = path;
    ...
    getShortenedOptionDefinitions: function getShortenedOptionDefinitions() {
      ...
      definitions[ArgumentParser.OUTPUT_FILE_SHORT_NAME]    = [ArgumentParser.LONG_OPTION_PREFIX + ArgumentParser.OUTPUT_FILE_NAME];
...
ArgumentParser.OUTPUT_FILE_NAME                          = 'out';
ArgumentParser.OUTPUT_FILE_SHORT_NAME                    = 'o';
ArgumentParser.DEFAULT_OUTPUT_FILE_VALUE                 = undefined;
